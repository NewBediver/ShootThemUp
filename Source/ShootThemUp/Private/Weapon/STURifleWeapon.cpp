...
void ASTURifleWeapon::Zoom(bool enabled) {
    ...
    if (enabled) {
        default_camera_fov_ = controller->PlayerCameraManager->GetFOVAngle();
    }

    controller->PlayerCameraManager->SetFOV(enabled ? fov_zoom_angle_ : default_camera_fov_);
}
...
void ASTURifleWeapon::MakeDamage(const FHitResult& hit_result) {
    ...
    FPointDamageEvent point_damage_event;
    point_damage_event.HitInfo = hit_result;
    damaged_actor->TakeDamage(damage_amount_, point_damage_event, GetController(), this);
}
