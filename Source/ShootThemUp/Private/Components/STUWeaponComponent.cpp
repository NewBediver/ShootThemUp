...
bool USTUWeaponComponent::GetAmmoData(FAmmoData& ammo_data) const {
    if (current_weapon_ != nullptr) {
        ammo_data = current_weapon_->GetAmmoData();
        return true;
    }
    return false;
}
...
bool USTUWeaponComponent::NeedAmmo(TSubclassOf<ASTUBaseWeapon> weapon_type) {
    for (const auto& weapon : weapons_) {
        if (weapon != nullptr && weapon->IsA(weapon_type)) {
